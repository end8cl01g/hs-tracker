...
import { X, Cloud, CloudOff, RefreshCw, Plug, Copy, Download, Upload, Trash2, ScrollText, Check, AlertTriangle } from 'lucide-react';
import {
  buildTag, checkGasUrl, copyText, diagnostics, exportBackup, getSetting, importBackup,
  pushBackupToCloud, reloadConfig, resetLocal, setSetting, subscribeSync, syncNow, testConnection,
  toast, type SyncState, EMPTY_SYNC, SYNC_LABEL,
} from '../store';
import { skyrimAudio } from '../utils/audio';

/**
 * 設定卷軸 —— 舊前端的設定頁搬進來（同步開關／GAS 連線／備份／重置／複製診斷）。
 * 這裡不實作任何同步或存檔邏輯，一律叫 store.ts 的接縫去使喚引擎模組，
 * 免得「兩套真值」：本機 DB 只有一份，介面只是它的窗。
 */
...
const STATUS_TONE: Record<SyncState['status'], string> = {
  init: 'text-stone-400', disabled: 'text-stone-400', syncing: 'text-sky-300', retrying: 'text-sky-300',
  ok: 'text-emerald-300', partial: 'text-amber-300', error: 'text-rose-300',
};

export const SkyrimSettings: React.FC<{ open: boolean; onClose: () => void; onChanged?: () => void }> =
  ({ open, onClose, onChanged }) => {
    ...
    useEffect(() => { if (!open) return; return subscribeSync(setSync); }, [open]);
    useEffect(() => {
      if (!open) return;
      (async () => {
        setUrl(await getSetting('gas_url'));
        setSecret(await getSetting('gas_secret'));
      })();
    }, [open]);
    useEffect(() => {
      if (!open) return;
    ...
    const act = async (name: string, fn: () => Promise<unknown>, okMsg?: (r: any) => string) => {
      setBusy(name);
      try {
        const r = await fn();
    ...
    const saveUrl = () => act('url', async () => {
      const { clean, problem } = checkGasUrl(url);
      setUrlProblem(problem);
      setUrl(clean);
      if (problem) throw new Error(`URL 不收：${problem}`);
      await setSetting('gas_url', clean);
      return clean;
    }, (c: any) => (c ? '雲端 URL 已存，按「測試連線」驗證' : '已清空 → 回到純離線模式（同步開關等於關）'));

    const saveSecret = () => act('secret', async () => {
      await setSetting('gas_secret', secret.trim());
      return null;
    }, () => '密鑰已更新（只存本機，不進任何網頁請求）');

    const doExport = () => act('export', () => exportBackup(), (r: any) => (r?.saved ? `備份檔已產出：${r.name}` : '備份取消'));
    const doCloudBackup = () => act('cloud-backup', () => pushBackupToCloud(), () => '已請雲端多留一份備檔');
    const doImport = (f: File | undefined) => {
      if (!f) return;
      void act('import', () => importBackup(f), () => '匯入完成，介面已重抓').then(() => fileRef.current && (fileRef.current.value = ''));
    };
    ...
    const doDiag = () => act('diag', async () => {
      ...
      toast(copied ? '診斷已複製到剪貼簿，貼回給我就行' : '剪貼簿不可用——下面這段手動複製');
      return text;
    });
    const doReset = () => act('reset', async () => {
      const ok = await resetLocal();
    ...
    const dot = (
      <span className={`inline-block w-2 h-2 rounded-full mr-2 align-middle ${
        sync.status === 'ok' ? 'bg-emerald-400' : sync.status === 'partial' ? 'bg-amber-400'
          : sync.status === 'error' ? 'bg-rose-400' : sync.status === 'init' || sync.status === 'disabled' ? 'bg-stone-500' : 'bg-sky-400 animate-pulse'
      }`} />
    );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center px-2 sm:px-4 py-4 sm:py-8">
        ...
        <div
          ...
          style={{ fontFamily: '"Cinzel","Noto Serif TC","Songti TC",serif' }}
        >
          <div className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 bg-gradient-to-b from-[#1c1a16] to-[#141311] border-b border-amber-900/40">
            ...
            <div className="ml-auto flex items-center gap-3">
              <span className="font-mono text-[10px] text-stone-500">build {buildTag().slice(0, 7)}</span>
          ...
          <div className="p-4 sm:p-6 space-y-7">
            {/* ── 同步狀態：三態一定要看得见（ok / partial / error），否則佇列會被藏起來 ── */}
            <section>
              ...
              <div className="border border-stone-700/60 bg-black/30 p-3.5">
                ...
                <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-1.5 text-[11px] text-stone-400">
                  <div>待同步 <span className="text-stone-200 font-mono">{sync.pending}</span> 列</div>
                  <div>衝突 <span className="text-stone-200 font-mono">{sync.conflicts}</span></div>
                ...
                <div className="mt-3.5 flex flex-wrap gap-2">
                  <Btn icon={<RefreshCw className={`w-4 h-4 ${busy === 'sync' ? 'animate-spin' : ''}`} />} onClick={doSync} disabled={!!busy}>立即同步</Btn>
                  <Btn icon={<Plug className="w-4 h-4" />} onClick={doPing} disabled={!!busy}>測試連線</Btn>
                  <Btn icon={<Cloud className="w-4 h-4" />} onClick={doCloudBackup} disabled={!!busy}>雲端備檔</Btn>
                  <Btn icon={<RefreshCw className="w-4 h-4" />} onClick={() => act('config', () => reloadConfig(), () => '設定檔已重抓')} disabled={!!busy}>重抓設定檔</Btn>
                </div>
                {pingInfo && <div className="mt-2.5 text-[11px] font-mono text-stone-400 break-all">{pingInfo}</div>}
              </div>
            </section>

            {/* ── 連線設定：URL 校驗在 GASProxy，只有一份規則 ── */}
            <section>
              <Label zh="連線凭证" en="ENDPOINT" />
              <div className="space-y-3">
                <div>
                  <div className="text-[11px] text-stone-500 mb-1">GAS Web App URL（清空並儲存＝停用雲端，回到純離線）</div>
                  <input className={field} value={url} onChange={(e) => { setUrl(e.target.value); setUrlProblem(null); }}
                    placeholder="https://script.google.com/macros/s/…/exec" spellCheck={false} autoComplete="off" />
                  {urlProblem && <div className="mt-1.5 text-[11px] text-rose-300">不收：{urlProblem}</div>}
                </div>
                <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Btn icon={<CloudOff className="w-4 h-4" />} onClick={() => { setUrl(''); void act('url-off', async () => { await setSetting('gas_url', ''); return true; }, () => '已停用雲端同步'); }} disabled={!!busy}>停用雲端</Btn>
                  <Btn icon={<Check className="w-4 h-4" />} onClick={saveSecret} disabled={!!busy}>儲存密鑰</Btn>
                </div>
              </div>
            </section>

            {/* ── 備份：離線優先 App 的自救通道（iOS 7 天會踢資料庫） ── */}
            <section>
              ...
              <div className="mt-2 text-[11px] leading-relaxed text-stone-500">
                匯入是「整庫取代」，不是合併。App 離線可用，但瀏覽器可能在 7 天後清掉本機資料庫——手動留一份才回得来。
              </div>
            </section>
            ...
            <section>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-[11px] text-stone-500">含版號、SW 是否接管、實際存到的 URL、ping 原文（不含密鑰本體）</span>
              </div>
              {diag && (
