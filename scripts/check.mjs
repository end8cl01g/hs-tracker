...
for (const a of assets) T(existsSync(join(ROOT, a)), `index.html → ${a}`, `index.html 引用的 ${a} 不存在`);

// 2) 前端不得有外部 CDN / fonts（離線優先的硬條件）
//    先剝掉註解再掃：註解裡「提到 cdnjs」是文件說明，不是依賴。
const stripComments = (t) => t.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/(^|[^:'"\`])\/\/[^\n]*/g, '$1');
const frontFiles = ['index.html', 'sw.js', 'manifest.json', ...walk('js'), ...walk('css')];
...
T(/navigator\.serviceWorker\.register\(/.test(read('js/app.js')), 'app.js 有 serviceWorker.register()', '找不到 SW 註冊：離線與 PWA 安裝性都會失效');

// 4) PRECACHE 的每一項都要存在（少一個檔 → cache.addAll 整批 reject → SW 永遠 install 失敗）
const sw = read('sw.js');
const precache = [...sw.matchAll(/'\.\/([^']+)'/g)].map((m) => m[1]);
const ghosts = precache.filter((p) => !existsSync(join(ROOT, p)));
...
const shellAssets = frontFiles.filter((f) => (f.endsWith('.js') || f.endsWith('.css')) && f !== 'sw.js').concat(['manifest.json', 'index.html']);
const notCached = shellAssets.filter((f) => !precache.includes(f) && !f.includes('build-info'));
...
const gsFiles = readdirSync(join(ROOT, 'gas')).filter((f) => f.endsWith('.gs'));
const defs = {};
for (const f of gsFiles) for (const m of read('gas/' + f).matchAll(/^function ([A-Za-z_$][\w$]*)\s*\(/gm)) (defs[m[1]] = defs[m[1]] || []).push(f);
...
const gsAll = gsFiles.map((f) => read('gas/' + f)).join('\n');
